package com.everis.accounts.app.config;
...
public class SwaggerConfig {
  ...
  public Docket api() {
    return changeGlobalResponses(new Docket(DocumentationType.SWAGGER_2).select()
        .apis(RequestHandlerSelectors.basePackage("com.everis.accounts.app")).paths(PathSelectors.any()).build()
        .apiInfo(metaData()));
  }

  @Bean
  public ApiInfo metaData() {
    return new ApiInfoBuilder().title("API REST ACCOUNTS").description("\"API REST RESPUESTA CUENTAS INFO\"")
